...
/// The `ChainSpec` parametrised for the acuity runtime.
pub type AcuityChainSpec = service::GenericChainSpec<acuity::GenesisConfig, Extensions>;
...
fn acuity_staging_testnet_config_genesis(wasm_binary: &[u8]) -> acuity::GenesisConfig {
	...
	acuity::GenesisConfig {
		...
		pallet_indices: Some(acuity::IndicesConfig { indices: vec![] }),
		...
		pallet_authority_discovery: Some(acuity::AuthorityDiscoveryConfig { keys: vec![] }),
...
pub fn acuity_staging_testnet_config() -> Result<AcuityChainSpec, String> {
	...
	Ok(AcuityChainSpec::from_genesis(
		...
		boot_nodes,
		Some(
			TelemetryEndpoints::new(vec![(ACUITY_STAGING_TELEMETRY_URL.to_string(), 0)])
